...
for gid in obj_groups:

    if gid not in ref_groups:
        continue
    ...
    print(f"\nProcessing set {gid}")
    print(f"{len(objs)} objects x {len(refs)} references")

    for obj_name, ref_name in tqdm(list(product(objs, refs))):

        obj_path = os.path.join(OBJ_DIR, obj_name)
        ref_path = os.path.join(REF_DIR, ref_name)

        pair_name = f"{Path(obj_name).stem}__{Path(ref_name).stem}"
